...
from personal_ppg2ecg.v4.metrics import calculate_fd_for_small_sample, ecg_bpm_array, heart_rate_metrics, zscore_numpy
...
def evaluate(samples_dir, sampling_rate=125, normalize=True, workers=8, seed=42, output_json=None):
    samples_dir = Path(samples_dir).resolve()
    output_json = (Path(output_json).resolve() if output_json else samples_dir / 'five_metrics.json').resolve()
    if ROOT not in output_json.parents:
        raise ValueError('Evaluation report must stay inside v4; use --output_json')
    if not output_json.parent.is_dir() or sampling_rate <= 0 or workers < 1:
    ...
    if fake.ndim == 2:
        fake = fake[..., None]
    if gt.shape != fake.shape or gt.ndim != 3 or len(gt) < 64:
        raise ValueError('Expected matching [N,L,C] arrays with at least 64 samples')
    if not np.isfinite(gt).all() or not np.isfinite(fake).all():
        raise ValueError('Nonfinite input signals')
    print(f'\n===== {samples_dir} =====\nGT shape {gt.shape} | Fake shape {fake.shape}', flush=True)
    start, count = time.monotonic(), len(gt)
