...
from personal_ppg2ecg.v3.metrics import calculate_fd_for_small_sample, ecg_bpm_array, heart_rate_metrics, zscore_numpy
...
def evaluate(samples_dir, sampling_rate=125, normalize=True, workers=8, seed=42, output_json=None):
    ...
    if ROOT not in output_json.resolve().parents:
        raise ValueError('Evaluation report must stay inside v3; use --output_json')
    ...
    print('Computing FD: all windows, PCA=64, eps=1e-4, 3 trials...', flush=True)
