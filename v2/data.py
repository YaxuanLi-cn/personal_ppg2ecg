...
def split_indices(subjects, seed, validation_size):
    if validation_size < 1 or validation_size >= len(subjects):
    ...
    for idx in np.random.default_rng(seed).permutation(len(subjects)):
        ...
        if len(validation) == validation_size:
            break
    if len(validation) != validation_size:
        raise ValueError('Not enough windows to retain distinct same-subject references')
...
class PairedDataset(Dataset):
    def __init__(self, data, stats, targets, reference_pool, seed=42, random_references=False):
        ...
        self.groups = subject_groups(self.subjects, np.sort(reference_pool))
        ...
        self.refs = None if random_references else reference_indices(
            self.subjects, self.targets, np.sort(reference_pool), np.random.default_rng(seed))
    ...
    def __getitem__(self, item):
        ...
        return (self.normalized('PPG', idx), self.normalized('ECG', idx),
                self.normalized('PPG', ref), self.normalized('ECG', ref))
