...
from personal_ppg2ecg.baseline.UniCardio.down_stream_code.AF.self_process import imputation_pattern
#%%
all_results = []
all_labels = []
all_truth = []
...
for i in range(0, 18):
    results_name = f'/root/autodl-tmp/BioDiffuser/AF/imputed_0.5_{i}.pt'
...
class VGGEcg(nn.Module):
    ...
    def forward(self, truth, results):
        ...
        # Concatenate the processed features from both modalities
...
for iteration in range(10):
    ...
    for train_index, test_index in kf.split(all_results):
        ...
        X_test_truth, mask = imputation_pattern(X_test_truth[:,None,:], extended_percentage = 0.5, extended = True)
        X_test_truth = X_test_truth[:,0,:]
    ...
    ACC[iteration] = average_accuracy
    SENS[iteration] = average_sens
    SPEC[iteration] = average_spec
print("ACC Mean: ", ACC.mean())
print("ACC STD: ", ACC.std())
print("SENS Mean: ", SENS.mean())
print("SENS STD: ", SENS.std())    
print("SPEC Mean: ", SPEC.mean())
print("SPEC STD: ", SPEC.std())    
