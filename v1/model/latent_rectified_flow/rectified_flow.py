...
class RectifiedFlow(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        if kwargs.pop("use_shared_private", False):
            raise ValueError("Shared/private mode requires PersonalRectifiedFlow and the existing patient encoder")
...
class PersonalRectifiedFlow(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.shared_feature_size = feature_size
        self.feature_size = private_dim if use_shared_private else feature_size
        self.temporal_size = temporal_size
        self.use_text = use_text
        self.train_num_points = train_num_points
        self.global_step = 0
        ...
        self.model = PersonalTransformer(
            n_feature=self.feature_size,
            condition_feature_size=feature_size if use_shared_private else None,
            n_temporal=temporal_size,
            n_layer_enc=n_layer_enc,
            n_embd=d_model,
            n_heads=n_heads,
    ...
    def output(self, x, t, padding_masks=None, cond=None, personal_cond=None, report=None):
        """Model forward pass"""
        if self.use_shared_private and (cond is None or personal_cond is None):
            raise ValueError("Private ECG flow requires both shared PPG and existing patient conditions")
        text_emb = self.get_text_embed(report) if self.use_text and report is not None else None
        t = t.view(-1, 1).repeat(1, x.shape[1])  # expand time dimension to match input
