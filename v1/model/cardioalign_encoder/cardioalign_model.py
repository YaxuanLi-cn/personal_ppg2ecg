...
class ECGPrivateEncoder(nn.Module):
    def __init__(self, private_dim=4, hidden_dim=64):
        super().__init__()
        if private_dim < 1 or hidden_dim < 32 or hidden_dim % 32:
            raise ValueError("private_dim must be positive; hidden_dim must be a multiple of 32")
        self.private_dim = private_dim
        self.hidden_dim = hidden_dim
        self.blocks = nn.Sequential(
            nn.Conv1d(1, hidden_dim, kernel_size=15, stride=5, padding=7),
            VAE_ResidualBlock(hidden_dim, hidden_dim),
            nn.Conv1d(hidden_dim, hidden_dim, kernel_size=17, stride=5, padding=8),
            VAE_ResidualBlock(hidden_dim, hidden_dim),
            nn.GroupNorm(32, hidden_dim),
            nn.SiLU(),
            nn.Conv1d(hidden_dim, private_dim, kernel_size=1),
        )

    def forward(self, ecg):
        return self.blocks(ecg.transpose(1, 2))
...
class ECGDecoder(VAE_Decoder):
    def __init__(self, private_dim=4, **kwargs):
        super().__init__(**kwargs)
        if private_dim < 1:
            raise ValueError("private_dim must be positive")
        self.private_dim = private_dim
        self.shared_dim = self.residual_decoder.blocks[0].in_channels
        self.fusion = nn.Conv1d(self.shared_dim + private_dim, self.shared_dim, kernel_size=1)

    def forward(self, shared_latent, private_latent):
        if shared_latent.ndim != 3 or private_latent.ndim != 3:
            raise ValueError("ECGDecoder expects [batch, channels, time] latents")
        if shared_latent.shape[::2] != private_latent.shape[::2]:
            raise ValueError("Shared and private latents must have matching batch and temporal dimensions")
        if shared_latent.shape[1] != self.shared_dim or private_latent.shape[1] != self.private_dim:
            raise ValueError("Latent channels do not match the configured shared/private dimensions")
        return super().forward(self.fusion(torch.cat((shared_latent, private_latent), dim=1)))


def shared_private_decorrelation(shared_latent, private_latent):
    if shared_latent.ndim != 3 or private_latent.ndim != 3:
        raise ValueError("Decorrelation expects [batch, channels, time] latents")
    if shared_latent.shape[0] != private_latent.shape[0] or shared_latent.shape[0] == 0:
        raise ValueError("Decorrelation requires matching nonempty batches")
    shared = shared_latent.float().mean(dim=-1)
    private = private_latent.float().mean(dim=-1)
    shared = F.normalize(shared - shared.mean(dim=0, keepdim=True), dim=-1)
    private = F.normalize(private - private.mean(dim=0, keepdim=True), dim=-1)
    covariance = shared.T @ private / shared.shape[0]
